...
## Dark SVG is getting generated below
...
# Create the SVG drawing
dwg = svgwrite.Drawing("static/images/books_read_per_year_dark.svg", size=(width, height))

# Create the chart area
chart_area = dwg.rect((chart_margin, chart_margin), (chart_width, chart_height), stroke="white", fill="black")
dwg.add(chart_area)
...
for i, year in enumerate(book_counts.keys()):
    x = chart_margin + (i * (chart_width / len(book_counts))) + ((chart_width / len(book_counts)) / 2) - 10
    y = chart_margin + chart_height + 20
    label = dwg.text(year, insert=(x, y), fill="white", font_size="14px", font_family="sans-serif", text_anchor="middle")
    dwg.add(label)

# Add the y-axis label
label = dwg.text("Number of Books Read", insert=(chart_margin - 40, chart_margin + (chart_height / 2)), fill="white", font_size="14px", font_family="sans-serif", text_anchor="middle", transform="rotate(-90, {0}, {1})".format(chart_margin - 40, chart_margin + (chart_height / 2)))
dwg.add(label)

# Add the chart title
title = dwg.text("Books Read Per Year", insert=(width / 2, chart_margin - 10), fill="white", font_size="24px", font_family="sans-serif", text_anchor="middle")
dwg.add(title)
